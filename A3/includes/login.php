...
if (isset($_REQUEST['submit-login-info'])) {
  ...
  $password = sanitizeData($_REQUEST['i-password']);
	...
	$sql = "SELECT * FROM `cb_login` WHERE cb_login_email='$email' AND cb_login_password='$password'";
	$result = $db->query($sql);
    ...
    $_SESSION['email'] = $email;
		...
		// DEBUGGING: so this statement was causing problems, we will see what the problem is later

		//getting user name
	  $sql2 = "SELECT * FROM `cb_users` WHERE cb_user_login_id='$loginId'";
...
<main class="container text-center"  >
  ...
  <form method="post"  action="login.php">
    ...
    <div class="form-floating">
      <input type="email" name="i-email" class="form-control" id="floatingInput" placeholder="name@example.com">
      <label for="floatingInput">Email address</label>
    </div>
    <div class="form-floating">
      <input type="password" name="i-password" class="form-control" id="floatingPassword" placeholder="Password">
      <label for="floatingPassword">Password</label>
    </div>
    <button class="w-100 btn btn-lg btn-primary" name="submit-login-info" value="Log in" type="submit">Log in</button>
